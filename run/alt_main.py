...
#         EDD_alt(m, n, h, machine_infos, job_dic)
...
#     cross_machine = True
#     org_time = time.time()
#     tabu_obj, best_maint_job_schedule, best_maint_loss = Tabu_alt(h, tabu_size, loop, cross_order, cross_machine, base_maint_loss, job_schedule, loss, cu_loss, job_dic, machine_infos)
...
# In[2]
def das_algorithm(file_num = 0, m_list = [], n_list = [], folder_name = "testdata", result_name = "result", tabu_size = [100], loop=100):
    for m in m_list:
        for n in n_list:
            tabu_dic = {}
            ...
            # 寫入 20 次測資結果
            with pd.ExcelWriter(result_name + "/m" + str(m) + "n" + str(n) + "_edd.xlsx", engine="openpyxl", mode='w') as writer:  
            ...
            with pd.ExcelWriter(result_name + "/m" + str(m) + "n" + str(n) + "_tabu.xlsx", engine="openpyxl", mode='w') as writer:  
