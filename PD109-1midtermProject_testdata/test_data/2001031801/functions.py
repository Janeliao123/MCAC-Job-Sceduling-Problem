...
def alt_decide_maintenance(decrease_rate,job_schedule, cu_loss, loss, h, pro_rate, maintain_time, job_dic, L_yield_rate):  # 最小化欠貨量
    maintenance_schedule = []  #紀錄修過的紀錄 (機台,時間)
    ...
    while flag == 1:  # 每個時段最多可以修 h 台
        ...
        for machine in range(len(job_schedule)):  # 每台機台都跑過
            ...
            for job_no,time,yr in new_job_schedule[machine_no]:  # 每台機台的每個地方插入維修，求最好的
                if index == 0:
                    temp_loss_list, temp_loss, temp_schedule = alt_insert_maintenance(time, maintain_time[machine_no], pro_rate[machine_no], new_job_schedule[machine_no][index:], 
                                                            0, decrease_rate[machine_no], L_yield_rate[machine_no], job_dic)
                else:
                    temp_loss_list, temp_loss, temp_schedule = alt_insert_maintenance(time, maintain_time[machine_no], pro_rate[machine_no], new_job_schedule[machine_no][index:], 
                                                            new_loss[machine_no][index-1][1], decrease_rate[machine_no], L_yield_rate[machine_no], job_dic)
                
                if temp_loss < m_loss:
                    flag = 1
                    m_index = index
                    m_improve_loss = m_loss - temp_loss
                    m_loss = temp_loss
                    m_loss_list = temp_loss_list
                    m_schedule = temp_schedule
                    m_info = (machine_no,time)  # (維修機台, 維修時間)
                index += 1
            if m_improve_loss > best_improve_loss:
                best_index = m_index
                best_loss = m_loss
...
def alt_insert_maintenance(time, maintain_t, pro_rate, schedule, loss, decrease_rate, L_yield_rate, job_dic):  # 在某個時間點插入維修重新規劃後的結果
    ...
    for job_n,t,_ in schedule:  # 剩下還沒有排到的工作
        ...
        if temp_load > 0:  # 未滿足交期
            new_loss += temp_load  # 只有太晚做完才要算 cost
