...
# Tabu 
def Tabu_alt(h, tabu_size, loop, cross_order, cross_machine, maintain_time, base_maint_loss, job_schedule, pro_rate, loss, decrease_rate, L_yield_rate, cu_loss, maint_sche, job_dic):
        ...
        for (job_1,job_2) in neighbor_pair:  # 更新所有的 neightboring pair
            ...
            # if (job_1,job_2) or (job_2,job_1) not in tabu_list: # 沒有換過，才要換換看
                    ...
                    for job_n, t, init_yr in swap_job_schedule[machine]:
                        if job_n == job_1:
                            ...
                            new_loss_list_1, new_loss_1, new_schedule_1 = \
                                reschedule_alt(t, init_yr, job_2, pro_rate[machine], swap_job_schedule[machine][index+1:],
                                before_loss, decrease_rate[machine], L_yield_rate[machine], job_dic)
                            ...
                            new_loss_list_2, new_loss_2, new_schedule_2 = \
                                reschedule_alt(t, init_yr, job_1, pro_rate[machine], swap_job_schedule[machine][index+1:],
                                before_loss, decrease_rate[machine], L_yield_rate[machine], job_dic)
                        index += 1
                # 將交換排程紀錄下來
                ...
                temp_maint_sche, temp_new_cu_loss, temp_new_loss, temp_new_job_schedule = \
                    alt_decide_maintenance(decrease_rate, temp_job_schedule, temp_cu_loss, temp_loss, h, pro_rate, maintain_time, job_dic, L_yield_rate)
    ...
    best_maint_sche, best_maint_cu_loss, best_maint_loss, best_maint_job_schedule = \
        decide_maintenance(decrease_rate, best_job_schedule, best_cu_loss, best_loss, h, pro_rate, maintain_time, job_dic, L_yield_rate)
